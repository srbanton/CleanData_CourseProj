...
For more detailed info on the signal processing visit the dataset's repository [here](http://archive.ics.uci.edu/ml/datasets/Human+Activity+Recognition+Using+Smartphones).     
...
1. Rename activity labels using factor()
...
##Descriptions of Variables in Tidy Dataset
...
**All features are normalized and bounded within [-1,1].  They do not have any units.**  Body refers to each subject's motion as tracked by the Samsung Galaxy's accelerometer and gyroscope. The 79 features (in the order of their appearance in tidydata.txt) are:
